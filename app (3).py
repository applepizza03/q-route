...
def load_routes(origin, destination):
    routes = [
        {
            "route_id": "R1",
            "path": f"{origin} → Shanghai → {destination}",
            "mode": "Sea + Truck",
            "cost": 1200,
            "time": 8,
            "carbon": 900,
        ...
        {
            "route_id": "R2",
            "path": f"{origin} → Tokyo → {destination}",
            "mode": "Air + Truck",
            "cost": 1500,
            "time": 6,
            "carbon": 1100,
        ...
        {
            "route_id": "R3",
            "path": f"{origin} → Singapore → {destination}",
            "mode": "Sea",
            "cost": 1000,
            "time": 11,
            "carbon": 700,
        ...
        {
            "route_id": "R4",
            "path": f"{origin} → Busan → Vancouver → {destination}",
            "mode": "Sea + Rail",
            "cost": 1350,
            "time": 9,
            "carbon": 850,
        ...
        {
            "route_id": "R5",
            "path": f"{origin} → Taipei → {destination}",
            "mode": "Air",
            "cost": 1450,
            "time": 7,
            "carbon": 980,
...
def run_qaoa_sampling(scores, penalty=2.0, p=1, shots=512, maxiter=20):
    ...
    decoded_df = pd.DataFrame(decoded).sort_values(
        ["valid", "energy", "probability"],
    ...
    html_block(
        ...
        <div class="mission-grid">
            <div class="mission-card">
                <h3>1️⃣ 물류 상황 설정</h3>
                <p>
                출발지와 도착지를 입력하고, 항구 파업·유가 급등·탄소세 강화·긴급 배송 같은
...
def render_app_screen():
    ...
    origin = st.sidebar.text_input("출발지", "Seoul")
    destination = st.sidebar.text_input("도착지", "Los Angeles")
    ...
    if run_button:
        ...
        html_block(
            ...
            <div class="winner-card">
                ...
                QUBO 에너지가 낮을수록 비용·시간·탄소배출·리스크를 종합적으로 고려했을 때 더 좋은 경로입니다.
                </p>
            </div>
            """
        )
        ...
        with battle_col2:
            ...
            st.caption("QUBO 문제를 양자 회로로 변환한 뒤, 측정 결과 중 제약조건을 만족하고 에너지가 낮은 bitstring을 선택합니다.")
